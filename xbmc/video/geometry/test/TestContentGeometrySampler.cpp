...
{
  const ContentGeometryScan scan = CDVDFileInfo::ExtractContentGeometry(LetterboxedClip());
...
{
  const ContentGeometryScan first = CDVDFileInfo::ExtractContentGeometry(LetterboxedClip());
  const ContentGeometryScan second = CDVDFileInfo::ExtractContentGeometry(LetterboxedClip());
...
TEST(TestContentGeometrySampler, HonoursTheRequestedPointCount)
{
  ...
  const ContentGeometryScan scan =
      CDVDFileInfo::ExtractContentGeometry(LetterboxedClip(), sampling);
...
TEST(TestContentGeometrySampler, RefusesItemsThatCannotBeExtracted)
{
  ...
  const ContentGeometryScan scan = CDVDFileInfo::ExtractContentGeometry(stream);
...
TEST(TestContentGeometrySampler, MissingFileYieldsNoReading)
{
  ...
  const ContentGeometryScan scan = CDVDFileInfo::ExtractContentGeometry(missing);
