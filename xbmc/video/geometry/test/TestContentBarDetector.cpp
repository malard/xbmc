...
void ExpectRect(const DetectionResult& result, int x1, int y1, int x2, int y2)
{
...
TEST(TestContentBarDetector, CleanBarsEightBitLimited)
{
  ...
  ExpectRect(result, 0, 60, 640, 300);
...
{
  CSyntheticFrame frame(3840, 256, 8);
  frame.FillPicture(64, 192, 60, 220, 7);
  frame.FillRows(0, 64, frame.Black(), frame.Neutral(), frame.Neutral());
  frame.FillRows(192, 256, frame.Black(), frame.Neutral(), frame.Neutral());
  ...
  const DetectionResult result = DetectContentRect(frame.Ref());
  ExpectRect(result, 0, 64, 3840, 192);
  EXPECT_FALSE(result.degenerate);
}
...
{
  CSyntheticFrame frame(640, 256, 8);
  frame.FillPicture(64, 192, 60, 220, 7);
  frame.FillRows(0, 64, frame.Black(), frame.Neutral(), frame.Neutral());
  frame.FillRows(192, 256, frame.Black(), frame.Neutral(), frame.Neutral());
  ...
  const DetectionResult result = DetectContentRect(frame.Ref());
  ExpectRect(result, 0, 64, 640, 192);
}
...
TEST(TestContentBarDetector, UnspecifiedRangeAssumesLimitedAndCostsConfidence)
{
  ...
  ExpectRect(assumed, 0, 60, 640, 300);
...
TEST(TestContentBarDetector, DarkSaturatedPictureIsNotABar)
{
  ...
  ExpectRect(result, 0, 40, 640, 360);
...
TEST(TestContentBarDetector, FlatShadedDarkPictureIsIndistinguishableFromABar)
{
  ...
  ExpectRect(result, 0, 80, 640, 280); // truth is 40 / 320
  EXPECT_FALSE(result.degenerate);
}
...
TEST(TestContentBarDetector, SubtitleInBarDoesNotMoveTheRect)
{
  ...
  ExpectRect(result, expected.rect.x1, expected.rect.y1, expected.rect.x2, expected.rect.y2);
...
TEST(TestContentBarDetector, PillarboxOnly)
{
  ...
  ExpectRect(result, 80, 0, 560, 360);
...
TEST(TestContentBarDetector, WindowboxNeedsColumnsJudgedOverPictureRowsOnly)
{
  ...
  ExpectRect(result, 80, 40, 560, 320);
  EXPECT_FALSE(result.degenerate);
}
...
TEST(TestContentBarDetector, OffCentreBarsAreReportedNotCorrected)
{
  ...
  ExpectRect(result, 0, 40, 640, 280);
...
TEST(TestContentBarDetector, Subsampling422)
{
  ...
  ExpectRect(DetectContentRect(frame.Ref()), 0, 60, 640, 300);
}
...
TEST(TestContentBarDetector, RoiSelectsOneViewOfSideBySide)
{
  ...
  ExpectRect(other, 640, 0, 1280, 360);
}
