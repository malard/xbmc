...
namespace
{
/* Serves a real file from memory and, once told to stall, behaves like the
 * file cache over a frozen network flow: each read blocks for up to ten
 * seconds and then reports an error, with the position still short of the
 * length and eof never raised.
 */
...
public:
  ...
  int Read(uint8_t* buf, int buf_size) override
  {
    if (m_stalled.load())
    {
      for (int i = 0; i < 100 && m_stalled.load(); ++i)
    ...
    {
      // eof latches on a zero read at the end and clears on a seek, exactly
      // like CDVDInputStreamFile
      m_eofLatched = true;
      return 0;
    }
    const int count =
 ...
 * own read timer fires before any io error can surface, and its AVERROR_EXIT
 * must be read as pacing rather than as an abort ending playback. The stall
 * phase deliberately outlives that timer, so this test runs for ~30 seconds.
 *
...
TEST(TestDVDDemuxFFmpeg, StalledInputOutlivesTheReadTimer)
{
  ...
  while (std::chrono::steady_clock::now() - stallStart < 26s)
  {
    DemuxPacket* packet = demux.Read();
