...
order is preserved everywhere so that generated documents are stable.

Twelve enum types referenced by the schemas are registered at runtime from
C++ tables and therefore have no definition in types.json; placeholder
schemas flagged with "x-kodi-runtime-enum" are synthesized for them.
"""
...
def load_version(schema_dir=SCHEMA_DIR):
    """Return the API version string from version.txt, e.g. "13.15.0"."""
...
def load_service(schema_dir=SCHEMA_DIR):
    ...
    referenced = collect_refs([methods, types, notifications])
    for name in sorted(referenced - set(types)):
